...
public class GlobalFlock : MonoBehaviour
{
    ...
    void Update()
    {
        m_TimeLeft = m_DimTimer.getRemainingTime();

        /*if (m_DimTimer.getRemainingTime() <= 50 && m_DimTimer.getRemainingTime() >= 20) //normal Boids
        {
        ...
        else if (m_DimTimer.getRemainingTime() <= 20 && m_DimTimer.getRemainingTime() >= 10)
        { //decrease the width, closer to the player

        }
        else*/ if (m_DimTimer.getRemainingTime() <= 115 && m_DimTimer.getRemainingTime() >= 0)
        {//stop fish and make them look at player
            ...
            for (int i = 0; i < m_NumFish; i++)
            {
                ...
                m_LookRotation = Quaternion.LookRotation(m_Player.position);
                print(m_LookRotation);
                print( m_Player.position);
